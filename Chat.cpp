...
void Chat::regIn()
{
	...
	cout << endl;
	cout << "Hey " << currUser_->getName() << "Welcome to chat  " << endl << endl;
}
...
void Chat::logIn()
{
	...
	if (!currUser_)
	{
		while (!currUser_)
		{
			...
			cout << endl;
			cout << "Login failed... Supported operations: 1 - try another login, 2 - register to chat, 0 - exit" << endl;
			cout << "Your operation: ";
	...
	else
	{
		...
		{
			cout << "Password failed. Supported operations: 1 - try another password, 0 - exit" << endl;
			cout << "Your operation: ";
	...
	while (currUser_)
	{
		...
		while (op != "1" && op != "2" && op != "0")
		{
			...
			case SHOW:
				...
				break;
			case LOGOUT:
				cout << currUser_->getName() << "is logged out! " << endl << endl;
				currUser_ = nullptr;
				break;
			default:
...
void Chat::writeMessage()
{
	...
	cout << "Everyone in the chat - " << i << endl << endl;
	cout << "Enter number : ";
	cout << endl;
	...
	{
		cout << "There is no user with number. Try again!" << endl;
		cout << "User number: ";
		cin >> userNum;
		cin.clear();
		cin.ignore(numeric_limits<streamsize>::max(), '\n');
	...
	if (userNum == i)
	{
		...
		cout << "The message is sent from " << currUser_->getName() << "to everyone in the chat..." << endl << endl;
	}
	else if (userNum > 0 && userNum < i)
	{
		Message message = Message(currUser_->getName(), users_.at(userNum - 1).getName(), text);
		messages_.push_back(message);
		cout << "The message is sent from " << currUser_->getName() << "to " << users_.at(userNum - 1).getName() << "..." << endl << endl;
	}
}
...
void Chat::showMessages()
{
	...
	if (count == 0)
	{
		cout << "You've got no messages yet..." << endl << endl;
	}
}
